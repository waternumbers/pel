...
A continuous random variable $X$ follows the Lindley distribution, then the cumulative distribution function (cdf) with
parameter q is given by

\begin{equation}
G\left(x\right) = 1 - \frac{\exp\left(-qx\right)\left(1+q+qx\right)}{1+q} \quad x > 0, q > 0
\end{equation}

and the probability density function (pdf) as

\begin{equation}
g\left(x\right) = \frac{q^2}{q+1}\left(1+x\right)\exp\left(-qx\right) \quad x > 0, q > 0
\end{equation}
...
The power exponentiated Lindley (PEL) distribution is introduced in Eq (5) and (6) of \supercite{Rajitha2002}. The cumulative density function for a PEL distribution with shape parameters $a$ and $v$ along with scale parameter $q$ is given in Eq <ref> where

\begin{equation}
\phi\left(x\right) = \frac{\exp\left(-qx\right)\left(1+q+qx\right)}{1+q}
\end{equation}

\begin{eqnarray}
F\left(x\right) = \frac{ v^{\phi\left(x\right)^{a}} - 1 }{v-1} \\
x>0,a>0,q>0,v>0
\end{eqnarray}
