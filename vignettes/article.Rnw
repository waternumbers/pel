...
This provides a continous range for $v$ which aids the numerical optimisation used in maximum likihood estimation (see Sections~\ref{sec:sim}). 


Ensuring individual terms within the formulea can be evaluated for $v<1$ by not simplifying $\log\left(\frac{\log v}{v-1}\right)$ in (\ref{eq:loglike}).
